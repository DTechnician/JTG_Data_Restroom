with
    audit_landing as (        
        ...
        where coalesce(coalesce(ssh.rate,0)/ssh.quantity/sfm.sf_value, 0)::number(12,2) <> 0
    ),

    audit_fact as (
        select wo.workordernumber,  ssh.site_service_id, dt.date as scheduled_date, r.revenue
